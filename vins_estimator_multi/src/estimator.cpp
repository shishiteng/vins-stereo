...
double errAngle = 0.0025; //先固定，权重=1/0.0025=400;
...
Estimator::Estimator() : f_manager{Rs}
{
    ...
    memset(&ceres_summary, 0, sizeof(solver_summary));
}
...
void Estimator::optimization()
{
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            {
                ProjectionFactorSingleCam *f = new ProjectionFactorSingleCam(pts_i, pts_j, errAngle);
                ...
                {
                    ProjectionFactorMultiCam *f = new ProjectionFactorMultiCam(pts_i, pts_j_r, errAngle);
                    problem.AddResidualBlock(f,
                                             loss_function,
                                             para_Pose[imu_i],
                                             para_Pose[imu_j],
...

    {
        ...
        ceres_summary.report = summary.FullReport();
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
            ...
            for (auto &it_per_id : f_manager.feature)
            {
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ProjectionFactorMultiCam *f = new ProjectionFactorMultiCam(pts_i, pts_j_r, errAngle);
                            ResidualBlockInfo *residual_block_info2 = new ResidualBlockInfo(f,
                                                                                            loss_function,
                                                                                            vector<double *>{para_Pose[imu_i],
                                                                                                             para_Pose[imu_j],
                                                                                                             para_Feature[feature_index],
                                                                                                             ...
                                                                                                             para_Ex_Pose[i]},
                                                                                            vector<int>{0, 2});
                            marginalization_info->addResidualBlockInfo(residual_block_info2);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < NUM_OF_CAM; i++) //sst:这里到底要不要呢？
