#include "ProjectionFactorSingleCam.h"
...
ProjectionFactorSingleCam::Evaluate( const double* const* parameters, double* residuals, double** jacobians ) const
{
    ...
    if ( jacobians )
    {
        ...
        if ( jacobians[0] )
        {
            ...
            jaco_i2j.rightCols< 3 >( ) = ric.transpose( ) * Rj.transpose( ) * Ri * - Utility::skewSymmetric( pts_imu_i );
            // clang-format on
        ...
        if ( jacobians[3] )
        {
            ...
            jaco_ex.rightCols< 3 >( )
              = -tmp_r_l * Utility::skewSymmetric( pts_camera_i )
                + Utility::skewSymmetric( tmp_r_l * pts_camera_i )
